 ...
 *
 * <p>Rules: - MS-001: API module exists (ERROR) - MS-002: Core module exists (ERROR) - MS-003:
 * Facade module exists (ERROR) - MS-004: Common module exists (WARNING) - MS-005: SPI module exists
 * (WARNING)
 */
public class ModuleStructureValidator {
  ...
  public List<ValidationResult> validate(List<ModuleScanner.ModuleInfo> modules) {
    ...
    if (!foundLayers.contains(ModuleScanner.ModuleLayer.API)) {
      results.add(
          ValidationResult.error("MS-001", "API module is required but not found", "project"));
    }

    // MS-002: Core module exists
    if (!foundLayers.contains(ModuleScanner.ModuleLayer.CORE)) {
      results.add(
          ValidationResult.error("MS-002", "Core module is required but not found", "project"));
    }

    // MS-003: Facade module exists
    if (!foundLayers.contains(ModuleScanner.ModuleLayer.FACADE)) {
      results.add(
          ValidationResult.error("MS-003", "Facade module is required but not found", "project"));
    }
    ...
    if (!foundLayers.contains(ModuleScanner.ModuleLayer.COMMON)) {
      results.add(
          ValidationResult.warning(
              "MS-004", "Common module is recommended but not found", "project"));
    }

    // MS-005: SPI module exists (WARNING)
    if (!foundLayers.contains(ModuleScanner.ModuleLayer.SPI)) {
      results.add(
          ValidationResult.warning("MS-005", "SPI module is recommended but not found", "project"));
    }
