 ...
 *
 * <p>Rules: - DP-001: Facade depends on Core (ERROR) - DP-002: Core depends on API, SPI, Common
 * (ERROR) - DP-003: API does not depend on Core (ERROR) - DP-004: No circular dependencies (ERROR)
 */
public class DependencyValidator {
  ...
  private void validateFacadeDependsOnCore(
    ...
    List<ModuleScanner.ModuleInfo> facades =
        layerMap.getOrDefault(ModuleScanner.ModuleLayer.FACADE, List.of());
    List<ModuleScanner.ModuleInfo> cores =
        layerMap.getOrDefault(ModuleScanner.ModuleLayer.CORE, List.of());
  ...
  private void validateCoreDependsOnApiSpiCommon(
    ...
    List<ModuleScanner.ModuleInfo> cores =
        layerMap.getOrDefault(ModuleScanner.ModuleLayer.CORE, List.of());
    List<ModuleScanner.ModuleInfo> apis =
        layerMap.getOrDefault(ModuleScanner.ModuleLayer.API, List.of());
        ...
        if (api.hasDependency(core.artifactId())) {
          results.add(
              ValidationResult.error(
                  "DP-003",
                  String.format(
                      "API module '%s' must NOT depend on Core module '%s'",
      ...
      if (hasCircularDependency(module.artifactId(), moduleMap, visited, recursionStack)) {
        results.add(
            ValidationResult.error(
                "DP-004",
                String.format(
                    "Circular dependency detected involving module '%s'", module.artifactId()),
                module.getFullName()));
      }
    }
  }
