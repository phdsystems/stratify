...
/**
 * Outputs validation results to console with color coding by severity. Provides a summary at the
 * end.
 */
public class ConsoleReporter {
  ...
  private void printResults(List<ValidationResult> results) {
    // Group by severity
    Map<ValidationResult.Severity, List<ValidationResult>> grouped =
        results.stream().collect(Collectors.groupingBy(ValidationResult::severity));
  ...
  private void printSummary(List<ValidationResult> results) {
    ...
    long errors =
        results.stream().filter(r -> r.severity() == ValidationResult.Severity.ERROR).count();
    long warnings =
        results.stream().filter(r -> r.severity() == ValidationResult.Severity.WARNING).count();
    long infos =
        results.stream().filter(r -> r.severity() == ValidationResult.Severity.INFO).count();
