...
/**
 * Validates SEA compliance by scanning modules and running validators. Fails the build if
 * violations of ERROR severity are found.
 */
...
public class StratifyValidateMojo extends AbstractMojo {
  ...
  public void execute() throws MojoExecutionException, MojoFailureException {
    ...
    try {
      ...
      // Check for failures
      long errorCount =
          allResults.stream().filter(r -> r.severity() == ValidationResult.Severity.ERROR).count();
      long warningCount =
          allResults.stream()
      ...
      if (failOnWarning && warningCount > 0) {
        throw new MojoFailureException(
            "SEA validation failed with " + warningCount + " warnings (failOnWarning=true)");
      }
