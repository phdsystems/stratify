...
public final class JavaParserUtil {
  ...
  /**
   * Checks if a class extends a specific class.
   *
   * @param decl Class declaration
   * @param className Class name to check (simple name or fully qualified)
   * @return true if the class extends the specified class
   */
  public static boolean extendsClass(ClassOrInterfaceDeclaration decl, String className) {
    return getSuperclassName(decl)
        .map(
            superclass ->
                superclass.equals(className)
                    || superclass.endsWith("." + className)
                    || className.endsWith("." + superclass))
        .orElse(false);
  }
