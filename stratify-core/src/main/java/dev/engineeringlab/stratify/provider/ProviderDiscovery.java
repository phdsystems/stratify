 ...
 *
 * <ul>
 *   <li>ServiceLoader-based discovery (standard Java SPI)
 *   <li>Annotation-based filtering ({@link Provider} annotation)
 * </ul>
 *
 * <p>Example usage:
 *
...
public final class ProviderDiscovery {
  ...
  public static <P> List<P> discover(Class<P> providerType, ClassLoader classLoader) {
    ...
    for (P provider : loader) {
      try {
        ...
        log.warn("Failed to load provider of type {}: {}", providerType.getName(), e.getMessage());
      }
    }
   ...
   * @return populated registry
   */
  public static <P> ProviderRegistry<P> loadRegistry(
      Class<P> providerType, ClassLoader classLoader) {
    ...
    for (P provider : discovered) {
      ...
      if (annotation != null) {
        try {
          ...
          log.debug(
              "Registered provider: {} with priority {}", annotation.name(), annotation.priority());
        } catch (StratifyException e) {
          log.warn("Failed to register provider {}: {}", annotation.name(), e.getMessage());
        }
      } else {
        log.debug(
            "Skipping provider without @Provider annotation: {}", provider.getClass().getName());
      }
    }
  ...
  public static <P> Optional<P> findByName(Class<P> providerType, String name) {
    return discover(providerType).stream().filter(p -> name.equals(getProviderName(p))).findFirst();
  }
  ...
  public static <P> Optional<P> findBestProvider(Class<P> providerType) {
    return discover(providerType).stream()
        ...
        .max(Comparator.comparingInt(p -> p.getClass().getAnnotation(Provider.class).priority()));
  }
